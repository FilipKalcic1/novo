...
class APICapabilityRegistry:
    ...
    1. Initial discovery from Swagger metadata
    2. Runtime learning from API responses
    3. Persistent caching for fast startup
    """

    # Parameter name variations (for detection)
    PERSON_ID_VARIATIONS = {
        "personid", "person_id", "driverid", "driver_id",
        "assignedtoid", "assigned_to_id", "userid", "user_id"
    }

    VEHICLE_ID_VARIATIONS = {
        "vehicleid", "vehicle_id", "carid", "car_id"
    }

    TENANT_ID_VARIATIONS = {
        "tenantid", "tenant_id", "x-tenant", "x-tenant-id"
    }
    ...
    async def _discover_from_registry(self) -> None:
        ...
        This is the KEY innovation - we analyze each tool's:
        1. Parameters (does it have personId? Filter?)
        ...
        for op_id, tool in self.tool_registry.tools.items():
            ...
            # Analyze parameters
            param_names_lower = {p.lower() for p in tool.parameters.keys()}

            # Check PersonId support
            if param_names_lower & self.PERSON_ID_VARIATIONS:
                capability.supports_person_id = ParameterSupport.NATIVE
                logger.debug(f"{op_id}: Native PersonId support detected")
            elif "filter" in param_names_lower:
                # Has Filter param - might support PersonId via filter
                # We'll learn this at runtime
            ...
            # Check VehicleId support
            if param_names_lower & self.VEHICLE_ID_VARIATIONS:
                capability.supports_vehicle_id = ParameterSupport.NATIVE
            elif capability.supports_filter:
            ...
            if any(x in output_lower for x in ["assigned", "driver", "owner"]):
                capability.returns_user_specific_data = True
        ...
        if cap.supports_person_id == ParameterSupport.NATIVE:
            ...
            if tool:
                for param_name in tool.parameters.keys():
                    if param_name.lower() in self.PERSON_ID_VARIATIONS:
    ...
    def record_success(self, operation_id: str, params_used: Dict[str, Any]) -> None:
        ...
        # Learn from success: if PersonId was in params, it's supported
        params_lower = {k.lower(): k for k in params_used.keys()}

        if params_lower.keys() & self.PERSON_ID_VARIATIONS:
