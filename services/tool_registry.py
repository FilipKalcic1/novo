...
class ToolRegistry:
    ...
    # Context parameter schemas (loaded from config/context_param_schemas.json at startup)
    # These are class-level defaults, populated by _load_context_param_schemas()
    CONTEXT_PARAM_PATTERNS: Dict[str, Dict] = {}
    CONTEXT_PARAM_FALLBACK: Dict[str, str] = {}
    COMPILED_NAME_PATTERNS: Dict[str, List[re.Pattern]] = {}
    ...
    def __init__(self, redis_client=None):
        ...
        # Load context parameter schemas from config file
        self._load_context_param_schemas()

        logger.info("ToolRegistry initialized (v2.0)")

    def _load_context_param_schemas(self) -> None:
        """Load context parameter classification schemas from config file."""
        config_path = Path.cwd() / "config" / "context_param_schemas.json"

        if not config_path.exists():
            logger.warning(
                f"Context param config not found: {config_path}. "
                "Using hardcoded defaults."
            )
            self._init_default_context_patterns()
            return
        ...
        try:
            ...
            logger.info(f"Loading context param schemas from {config_path}")

            # Load context types
            context_types = config.get("context_types", {})

            for context_type, rules in context_types.items():
                classification_rules = rules.get("classification_rules", {})

                # Build pattern dict for _classify_context_parameter
                ...
                # Build fallback name map
                fallback_names = rules.get("fallback_names", [])
                for name in fallback_names:
                    self.CONTEXT_PARAM_FALLBACK[name.lower()] = context_type

            logger.info(
                f"Loaded {len(context_types)} context types, "
                f"{len(self.CONTEXT_PARAM_FALLBACK)} fallback names"
            )

        except Exception as e:
            logger.error(f"Failed to load context param schemas: {e}")
            logger.warning("Falling back to hardcoded defaults")
            self._init_default_context_patterns()

    def _init_default_context_patterns(self) -> None:
        """Initialize with hardcoded defaults (fallback if config missing)."""
        ...
        logger.info("Using hardcoded context param patterns (config file missing)")
    ...
    def _classify_context_parameter(
        ...
        """
        Classify parameter as context param using schema metadata + description.

        Returns:
            (context_key, is_context_param)

        Strategy:
        1. Check Swagger schema format (uuid, guid)
        2. Check description keywords (person, vehicle, tenant)
        3. Fallback to name matching for poorly documented APIs
        """
    ...
    def _parse_parameter(self, param: Dict) -> Optional[ParameterDefinition]:
        ...
        # Classify using schema metadata + description
        context_key, is_context = self._classify_context_parameter(
            param_name, param_type, param_format, description
        )

        dependency_source = DependencySource.FROM_CONTEXT if is_context else DependencySource.FROM_USER
    ...
    def _cosine_similarity(
        ...
        """Calculate cosine similarity. Delegates to scoring_utils."""
    ...
    def _apply_method_disambiguation(
        ...
        # Use centralized patterns from services.patterns
