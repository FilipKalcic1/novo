...
class MessageEngine:
    ...
    def __init__(
        ...
        # Load context parameter examples from registry config
        self._context_param_examples = self._load_context_param_examples()

    def _load_context_param_examples(self) -> List[str]:
        """
        Load parameter name variations from context_param_schemas.json.

        Returns list of parameter names like ['VehicleId', 'PersonId', 'vehicleid', 'personid'].
        Used for pattern matching in error messages.
        """
        ...
        # Use all fallback names from config (already loaded by ToolRegistry)
        # These come from config/context_param_schemas.json
        examples = list(self.registry.CONTEXT_PARAM_FALLBACK.keys())

        # If no examples found in config, use fallback
        if not examples:
    ...
    async def _execute_tool_call(
            ...
            if missing_param and chain_depth < MAX_CHAIN_DEPTH:
                ...
                if resolution.success:
                    ...
                    if hasattr(conv_manager.context, 'tool_outputs'):
                        ...
                        # Store feedback for user confirmation (FIX: use feedback field)
                        if resolution.feedback:
                            conv_manager.context.tool_outputs["_entity_feedback"] = resolution.feedback
        ...
        if exec_result.success:
            ...
            # FIX: Prepend entity feedback confirmation if available
    ...
    def _inject_person_filter(
        ...
        # Fallback: If capability registry not initialized, use schema-based detection
        # Uses param_def.context_key instead of hardcoded parameter names
        for param_name, param_def in tool.parameters.items():
            if param_def.context_key == "person_id":
                modified_params = parameters.copy()
                modified_params[param_name] = person_id
                logger.info(f"🎯 SCHEMA-BASED INJECT: {param_name}={person_id} for {tool.operation_id}")
                return modified_params
    ...
    def _extract_missing_param_from_error(self, error_message: str) -> Optional[str]:
        ...
        # Pattern 3: Look for context parameter patterns (loaded from config)
